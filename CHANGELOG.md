...
История выпусков `size-report`. Версии — SemVer (`PLAN.md` §8.2); версия данных —
...
- Удаление метрики `gzip` из реестра (`PLAN.md` §10, D4) и лицензионный файл
  (открытый вопрос §10).
- Поставка `dist/app.js` (пре-собранная программа отчёта), публикация пакета:
  сегодня установка идёт git-зависимостью из приватного репозитория, `PLAN.md` §8.4.
