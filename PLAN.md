...
кэш на диске и разделение быстрого и медленного наборов проверок; профиль на
репозитории в пару тысяч коммитов; `docs/ARCHITECTURE.md`, `docs/DATA-FORMAT.md`,
`docs/METHODS.md` — сегодня роль каждого числа описана в `README.md`; удаление
метрики `gzip` (D4) и лицензионный файл (§10).
