...
**Осталось из шага 2:** передача выбора ссылкой (`location.hash` — там, где памяти
нет или её надо передать коллеге), пре-собранный `dist/app.js` вместо текста в
движке и отказ от «отчёта в git» в проекте-потребителе. Внешний вид страницы и
дерево файлов сделаны раньше (`REFACTOR.md` R-2.2, R-2.4).
...
по шагам: `pnpm install --frozen-lockfile`; `lint:strict`; `pnpm test`; тот же
