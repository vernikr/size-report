...
|---|---|---|
| Имя пакета и способ публикации (приватно/публично) | ✅ решено 2026-09-15: имя **`@vernikr/size-report`** (область владельца — имя `size-report` в реестре занято чужим пакетом), установка пока git-зависимостью из публичного репозитория; публикация в реестр ждёт входа в npm — это шаг владельца | 1 |
...
**Чем платит установка из реестра — шаг владельца.** Публикация требует входа в
npm (`npm publish`), а учётных данных на машине нет вовсе: файл настроек npm
пустой, `npm whoami` отвечает `ENEEDAUTH`, токена в окружении и в цепочке ключей
нет. Поэтому в текстах и в `README.md` по-прежнему стоит git-ссылка: обещать
установку по имени из реестра, пока её нет, — то же «обещание без факта», что уже
вычищалось четыре раза (`REFACTOR.md` R-4.19, R-4.20). Строка установки из реестра
появится в том же коммите, что и подтверждённая публикация.
