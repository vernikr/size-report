...
3. **публичный npm** — если инструмент захочется показывать, требует проверки
   ...
   **другого имени**: `size-report` в реестре занят чужим пакетом (2017 год, три
   ...
   пока имя то же, набранное руками `npx size-report` в проекте без установленного
   пакета разрешается в чужой пакет из реестра (инструмент этого зова больше не
   советует — файл с пакетом в `node_modules` и подсказки говорят путём, `REFACTOR.md`
   R-4.21).
...
путаницы: имя, набранное руками, в проекте без установленного пакета разрешается
в чужой пакет, а не в отказ (тексты инструмента имя больше не называют — R-4.21).
...
отдельный шаг — зарегистрировать логин. Счёт ниже — вхождения строки `size-report`. Манифест:
