...
### Шаг 4. Токены (v0.4.0) — срез 1 ✅ **сделано 2026-09-14**
...
**Осталось из шага 4:** семейства кроме `openai` (только со своим словарём — иначе
помечать приближением), переключатель семейства на странице (сегодня страница
переключает метрику целиком), а пометка приближения **в каждой клетке** — та же
незакрытая часть шага 3 (`fallback: true` в точке, форма данных §4.3).
