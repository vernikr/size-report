...
3. **публичный npm** — если инструмент захочется показывать, требует проверки
   лицензий зависимостей (esbuild — MIT, словари токенизаторов — проверить) и
   **другого имени**: `size-report` в реестре занят чужим пакетом (2017 год, три
   версии, `npm view size-report`), поэтому под этим именем публикация невозможна;
   пока имя то же, `npx size-report` в проекте без установленного пакета
   разрешается в чужой пакет из реестра.
...
|---|---|---|
| Имя пакета и способ публикации (приватно/публично) | ✅ решено 2026-09-14: имя `size-report`, **git-зависимость из публичного репозитория** (приватным он был до 2026-09-14, §8.4) — проверена в свежем проекте и без учётных данных | 1 |
...
**Рекомендация — `@vernikr/size-report`**, если логин свободен: имя не путается с
чужим пакетом того же смысла, а меняется только форма вызова
(`npx @vernikr/size-report`), потому что исполняемый файл называется `size`, а не
именем пакета. Без регистрации логина — `size-report-cli`: свободен без условий и
ближе всех к смыслу, но остаётся соседом чужого `size-report`, а это и есть цена
путаницы: `npx size-report` в проекте без установленного пакета разрешается в чужой
пакет, а не в отказ.

**Что затронет переименование** — набор файлов у всех вариантов один и тот же,
различается только строка замены (`npx size-report` → `npx @vernikr/size-report` или
имя без области), а у области владельца к этому добавляется отдельный шаг:
зарегистрировать логин. Счёт ниже — вхождения строки `size-report`. Манифест:
поле `name` — и только оно, `bin` остаётся `size`. `src/` — 18 вхождений в семи
файлах; решают `src/tool.js` (имя в данных), `src/config.js` и `src/cli.js` (умолчание
и черновик команды починки — та самая строка, что попадает в подпись отчёта),
`src/refusal.js` (справка и подсказка), `src/hook.js` (путь к движку в каталоге
установки; место состояния и метка хука — свои литералы, их можно оставить, чтобы уже
поставленные хуки узнавались и снимались), `src/page/app.js` (имя входит в паспорт
отчёта, от которого зависят ключ памяти и якорь ссылки). Шаблоны — 14 вхождений,
включая имена файлов `templates/size-report.config.json` и
`.github/workflows/size-report.yml`. Проверки и инструменты — 49 вхождений; с именем
...
разрешённых зовов). Документация — 124 вхождения в семи файлах, но править надо не
...
ключи не нужны, версия держится тегом. Не даёт она трёх вещей — установки по имени
из реестра, штатного обновления версии и спокойного `npx size-report` в проекте без
установленного пакета (сегодня это имя ведёт в чужой пакет).
