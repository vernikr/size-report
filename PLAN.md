...
месте. Вычислительная часть страницы — отдельный текст (`APP_MATH`), который
прогоняет тест: его итоги сверяются с итогами артефакта, дельта итога — с суммой
дельт по файлам, а сумма дельт по колонке — с текущим размером. Страница работает в
настоящем DOM (jsdom) — и в тесте, и в браузере. Числа контракта сверяются с
замороженным эталоном и на фикстуре, и на живом проекте (`pnpm run parity:live`).
