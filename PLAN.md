...
└── test/
    ...
    └── *.test.js           # наборы: история, датчики, модель, отчёт, CLI, полнота
```

Фикстуры не публикуются в npm (`files` их не включает): они нужны тому, кто
работает с исходниками пакета, а не тому, кто поставил его зависимостью.
...
ESM и разбивается по файлам §4.2 ровно настолько, чтобы дальше не тащить монолит.
