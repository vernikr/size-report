...
✅ `size check` и `size explain` сделаны 2026-09-14 (`REFACTOR.md` R-4.12);
`init`/`measure`/`render` пока ключи, `doctor` и `install-hook` не сделаны.
...
в CI и переезд с уже лежавшей копии) — `WORKLOG.md` §18.
...
**Делаем:** `size doctor` (один JSON обо всём, что агент иначе собирает руками) и
`install-hook` с защитой от зацикливания.
...
| `docs` | ✅ есть: утверждения документации о репозитории — пути, таблица файлов ↔ дерево, команды и ключи из справки, число проверок и цели по времени, адреса разделов (`test/docs.test.js`) | Обещания будущего, формулировки и смысл — вне проверки, названо в её шапке |
...
- ✅ у обещаний самой документации есть сторож (`test/docs.test.js`,
  `REFACTOR.md` R-4.1): пути, таблица файлов, команды и ключи, числа и адреса
  разделов — машинно; остальное названо словами и остаётся человеку.
