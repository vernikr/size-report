...
- **D6.** Сверка с рабочим деревом сравнивает содержимое, а не байты
  (`BLOCKERS.md` §B2): при `core.autocrlf=true` инструмент ложно отказывается
  работать. Без правки механики не закрывается, поэтому отдельным проходом.
...
| `core.autocrlf` в выкладке файлов | Ложный отказ «перенос состояния пропустил правку»: на машинах с настройкой по умолчанию (Git для Windows) инструмент не работает | Открыто: `BLOCKERS.md` §B2, пункт D6 |
