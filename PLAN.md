...
10. **Гард стриппера**: результат упрощения JavaScript обязан компилироваться
    (`vm.Script`, для `export`-файлов — `node --check` через временный файл).
    Упрощение не имеет права молча выбросить что-то кроме комментариев и отступов.
11. **Сверка с рабочим деревом** (`assertMatchesDisk`): размер на HEAD обязан
    совпасть с файлом на диске, если файл не грязный, — это ловит потерю правки
    при переносе состояния между коммитами.
...
результат — в `test/fixtures/`.

Ход работ, который этот шаг описывает:

1. `node tools/size-table.js --write` и хеш `docs/size-table.html` — сохранить как
   `test/fixtures/parity/artifact.sha256` (шаг 1 обязан воспроизвести тот же файл
   **байт-в-байт**).
2. `node tools/size-table.js --json > test/fixtures/parity/data.json` на текущем
...
|---|---|---|
| `pnpm test` — 10 проверок, 6,5 с | Фикстура: `--json` побайтово, sha256 артефакта, контрольный режим, прогон под `LC_ALL=C` — для движка пакета и рядом для эталонной копии | 10 из 10 |
| `pnpm run parity:live` — 8,5 с | Живой проект на клоне: `--json` побайтово, sha256 артефакта, контрольный режим | 95 строк × 27 колонок, артефакт 225 673 Б, sha256 `1bdb27e1…` — совпало |

**Отложено этим проходом** (перенос не улучшаем — улучшения отдельными пунктами):

- **D1.** Разбиение движка по файлам §4.2 — отдельным проходом после паритета.
- **D2.** Чтение путей из git независимо от `core.quotePath`
  (`BLOCKERS.md` §B1): сейчас колонка с не-ASCII путём пустеет, а коммит, у
  которого она была единственным изменением объёма, теряет строку.
- **D3.** Текст подсказки в ошибке цитирует путь движка (`src/size-table.js`
  вместо `tools/size-table.js`) — числа и форма отчёта не затронуты
  (`BLOCKERS.md` §N2).
...
- **D5.** Шапка движка всё ещё предлагает запуск `node tools/size-table.js`:
  текст устарел после переезда, но его правка — уже правка файла, а не перенос.
  Идёт вместе с пунктом D1, когда файл всё равно переустраивается.
...
Три сценария, выбираем на шаге 1 (открытый вопрос §10):

1. **git-зависимость** — `"size-report": "github:vernikr/size-report#v0.1.0"`:
   ноль инфраструктуры, работает в CI, ставится как обычный пакет;
...
| Пути вне ASCII и `core.quotePath` | Колонка с не-английским путём молча пустеет, у коммита пропадает строка; результат зависит от настроек машины | Известный пробел `BLOCKERS.md` §B1: обойдён закреплением настройки в тесте, починка — пунктом D2 |
...
|---|---|---|
| Имя пакета и способ публикации (приватно/публично) | `size-report`, приватно (git-зависимость → GitHub Packages) | 1 |
...
- `BLOCKERS.md` — открытые блокеры и известные пробелы (сейчас — §B1).
