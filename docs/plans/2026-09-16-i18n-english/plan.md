...
| S2 | `settings.md` | `src/config.js` (35), `src/project.js` (4), `src/init.js` (18) — what a settings file says when it is wrong, and what `--init` prints; follows S1's cause names and advice markers, and owns `outsideFix`, which S4 prints | 57 | **in progress 2026-09-16** — steps 1–2 landed (`ea6b743` `src/config.js` 34 → 0 and the six catalogue cases; `src/init.js` 18 → 0, `tools/refusals.js` 78 → 77, the `config already exists` case moved with the text). Steps 3–4 and the wash-up remain (`src/project.js` still reads 4) |
