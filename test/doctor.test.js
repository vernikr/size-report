...
/* Умолчание обязано покрывать проект: колонка — каждый отслеживаемый файл, который
 * можно измерить, а `skip` — прочие. Выборка вместо всего проекта врёт о проекте
 * (называет его объём по горсти файлов) и заставляет человека разгребать разницу.
 * Проект здесь из трёх файлов и двух с одним расширением: прежний выбор «по одному
 * от расширения» оставил бы колонкой один — то есть проверка падает до правки. */
test('свежий проект без настроек: колонкой идёт каждый отслеживаемый файл', () => {
  ...
  gitIn(dir, ['add', '-A']);
  gitIn(dir, ['commit', '-qm', 'начало']);
  // Файл вне git — не файл проекта: колонкой он не становится и в дереве отчёта
  // стоит потому, что дерево берётся из путей проекта, а не с диска.
