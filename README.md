...
**У обещаний документации есть сторож** (`REFACTOR.md` R-4.1): `test/docs.test.js`
проверяет машинно — пути из текста есть в дереве, таблица файлов сходится с деревом
в обе стороны, команды и ключи инструкций есть в справке, числа проверок и целей
совпадают с фактом, ссылки на разделы ведут в существующие. Заведённый сторож сразу
...
**14,7–16,2 с** при 97 проверках — 14,2–15,7 с при 92 (шесть прогонов: 14,43 /
14,44 / 14,91 / 15,71 / 14,23 / 15,27 — последний в среде без настроек машины)
плюс **0,46 с** сторож документации; поэтому цель — `pnpm test`
**≤ 18 с** (запас 1,8 с). Почему поднята: девять новых проверок полноты и
...
набор подорожал именно на этих проверках, а не на машине. Числа сторожа сняты на
занятой машине (чужие сборки поднимали загрузку до 190, и та же ревизия шла 15,5 с
и 20,0 с подряд), поэтому бюджет собран из спокойного замера и стоимости нового
файла, а не снят как есть.
...
| `test/docs.test.js` | Сторож документации: названные пути есть в дереве, таблица файлов совпадает с ним в обе стороны, команды и ключи есть в справке, числа проверок и целей — факт, ссылки на разделы не ведут в пустоту |
