...
**Выпуск 1.2.0 (2026-09-15).** Инструмент живёт отдельным пакетом: имя в
реестре — `@vernikr/size-report` (публикуется тегом из CI, без секрета).
...
|---|---|---|
| Быстрый — каждая правка | `pnpm test` | **65 из 165** |
| Полный — выкладка и CI | `pnpm test:all` | **165** |

Ни одна проверка не потеряна и не ослаблена: полный прогон запускает все 165 теми же
...
| `src/init.js` | Черновик настроек для нового проекта: колонки по расширениям, журнал, менеджер пакетов — и он же проходит ту же проверку, что первый запуск |
...
version` отвечает `1.2.0`, `npm access get status @vernikr/size-report` — `public`,
...
движущаяся цель, а тег постоянен: этот выпуск стоит на теге `v1.2.0`, он же и в
