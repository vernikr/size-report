...
**Выпуск 1.1.0 (2026-09-15).** Инструмент живёт отдельным пакетом, версия — в
...
| `package.json` | Манифест пакета: версия `1.1.0`, список поставки — только существующее |
...
```bash
pnpm add -D github:vernikr/size-report#v1.1.0
```
...
пакета, затем `pnpm add -D ./size-report-1.1.0.tgz`.
...
движущаяся цель, а тег постоянен: этот выпуск стоит на теге `v1.1.0`, он же и в
