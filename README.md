...
дерево (32 файла) даёт ноль замечаний, `fixtures/` не линтуются — там данные.
...
добавлены только вместе с новым поведением страницы и подключения — всего 47.
Прогон подешевел втрое: `pnpm test` 15,7 → **5,6 с**, `pnpm run parity:live`
23,8 → **13,3 с**.
...
| `.gitignore`, `pnpm-lock.yaml` | Что в репозиторий не идёт; lock-файл pnpm (менеджер закреплён установкой, не текстом доки) |
...
(Подсказка `--init` обещает «проверки едут вместе с пакетом» — звучит так, будто
в пакете лежит тестовый набор; он остаётся в репозитории пакета, и формулировку
надо поправить — `REFACTOR.md` R-4.8.)
...
- **Разбор модуля стоит запуска Node на клетку**: файл-модуль на каждой ревизии,
  где он менялся, проверяется отдельным процессом. На истории в тысячу коммитов
  и десяток модулей это заметно (замер и предложение по починке — `REFACTOR.md`
  R-5.4); скриптовые проекты не платят ничего;
...
пути, которые README называет своими, сверены с деревом. Постоянной проверки
доков в пакете пока нет — это шаг 6 плана (`REFACTOR.md` R-4.1).
