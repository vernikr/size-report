...
добавлены только вместе с новым поведением страницы (42). Прогон подешевел
втрое: `pnpm test` 15,7 → **5,6 с**, `pnpm run parity:live` 23,8 → **13,3 с**.
...
```bash
pnpm add -D github:vernikr/size-report#cf7e6b5   # любой коммит или тег
```
...
| `minify.guard` | расширения, где результат стриппера проверяется компиляцией; для `.js`-модулей его надо сузить — см. «Ловушки» |
...
- **Модуль в расширении `.js`.** Если в колонках есть `.js` с `import`/`export`
  (обычное дело в проектах с бандлером), метрика `min` падает кодом 5: гард
  компиляции проверяет такой файл как скрипт (`REFACTOR.md` R-4.6). Пока не
  починено — уберите `.js` из гарда, оставив его для `.mjs`:
  `"minify": { "guard": [".mjs"] }`;
...
- **Подсказка про конфиг печатает длинный путь** к `bin/size.js` в `node_modules`
  вместо короткой команды (`REFACTOR.md` R-4.7), а в отчёте без `fixCommand`
  стоит старое имя пакета (`npx size-table --write`, R-4.8);
- **`--init` не правит `.gitignore`** (R-4.8) — добавьте отчёты руками, если им не
  место в истории.
