...
**15,2–16,7 с** при 99 проверках — 14,7–16,2 с при 97 (14,2–15,7 с при 92 плюс
0,46 с сторож документации) плюс **0,53 с** сторож границы git; поэтому цель — `pnpm test`
**≤ 18 с** (запас 1,3 с). Почему поднята: девять новых проверок полноты и
...
набор подорожал именно на этих проверках, а не на машине. Числа сторожей
сняты на занятой машине (чужие сборки поднимали загрузку до 190, и та же ревизия шла
15,5 с и 20,0 с подряд), поэтому бюджет собран из спокойного замера и стоимости
нового файла, а не снят как есть.
