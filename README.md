...
|---|---|---|---|
| Быстрый — каждая правка | `pnpm test` | **58 из 132** | **≤ 10 с** |
| Полный — выкладка и CI | `pnpm test:all` | **132** | **≤ 32 с** |
...
| `tools/refusals.js` | Каталог отказов: по строке на каждый — причина, код выхода, обязательные фразы вывода, а для непроверяемого — почему; карты мест отказа (`SITES`, `PRINTED`) держат числа, чтобы новый отказ не появился молча |
...
| `test/refusals.test.js` | Отказы исполняются: каждый вызван прогоном, сверены код выхода и обещанные фразы (свои клоны — для чужого хука, обрезанной истории и ветки мимо отчёта) |
| `test/refusals-catalog.test.js` | Сторож каталога отказов: у каждого места отказа в исходниках есть пункт, и отказы, отданные другой проверке, ею в самом деле утверждаются |
