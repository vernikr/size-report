...
**25,5–25,7 с** при 119 проверках (25,5 с в среде без настроек git) — это
24,3–25,7 с при прежних 117 плюс **+1,5–2,4 с** за две проверки этого прохода
(тринадцать запусков инструмента: отказы по словам и пример установки). Вклад
измерен парным прогоном тех же двух наборов с новыми проверками и без них —
4,18 → 6,54 с, — а не выведен из разброса прогонов: тот же набор в другом окне шёл
28,9 и 29,1 с при загрузке 6,7, то есть числа прогонов на занятой машине
сравнивать нельзя вовсе. Из общего времени **+8,5 с** — десять проверок хука
...
двигалась, и это решение, а не пропуск: цель — `pnpm test` **≤ 28 с** (запас
2,3 с) — измеренное в неё укладывается, а поднимают цель по делу и с измерением,
...
```bash
pnpm add -D github:vernikr/size-report#6530237ab826b011240ad8283a4ef499cc90bf6f
```
...
**Почему sha целиком.** Короткий sha pnpm разрешает только через видимые рефы, а
...
целиком поставился. Поэтому в примере сорок знаков. Имя ветки (`#main`) или тег
тоже принимаются, но ветка — движущаяся цель, а тег постоянен: тег будет на 1.0.0.
...
ревизией, в которой есть всё названное ниже — включая отказы на незнакомое слово. За этим следит сторож документации (`test/docs.test.js`): пин обязан вести
