...
Ещё не сделано — токены; минификация сделана первым срезом шага 3 (ниже).
...
выбора и ссылке на странице, а последние семь — о настоящем сжатии (§«Метрика
`min` умеет считать по-настоящему»); всего 66.
...
**7,9–8,0 с** при 66 проверках (три прогона: 7,89 / 7,96 / 7,85), поэтому цель —
...
(`"minify": {"engine": "esbuild"}`): минификатор едет необязательной
зависимостью и ставится обычной установкой, а без него инструмент работает и сам
скажет об этом (код 4) — правки настроек не требуются.
...
| `metrics` | из чего состоит число: `raw` (размер объекта git), `min` (минифицированная форма — какой именно, решает `minify.engine`), `gzip` |
