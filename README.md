...
**Выпуск 1.0.0 (2026-09-14).** Инструмент живёт отдельным пакетом, версия — в
манифесте, а у выпуска есть `CHANGELOG.md` с разделом «Что изменится в числах»:
таблица чисел в нём не пересказ, а замер на фикстуре, который сверяется с живым
прогоном (`test/changelog.test.js`). `schema: 1` данных этим выпуском объявлена
замороженной.
...
| `package.json` | Манифест пакета: версия `1.0.0`, список поставки — только существующее |
...
```bash
pnpm add -D github:vernikr/size-report#v1.0.0
```
...
тарболл: `pnpm pack` в клоне пакета, затем `pnpm add -D ./size-report-1.0.0.tgz`.
...
движущаяся цель, а тег постоянен: этот выпуск стоит на теге `v1.0.0`, он же и в
