...
**Выпуск 2.3.0 (2026-09-15).** Инструмент живёт отдельным пакетом: имя в
...
правки в том, **что именно считается**: без файла настроек колонкой идёт каждый
отслеживаемый файл (а не выборка из двенадцати), и в дереве панели теперь
складываются папки; `schema: 1` данных, замороженная 1.0.0, остаётся той же (выпуск
добавил поле, а не поменял смысл прежних).
...
|---|---|---|
| Быстрый — каждая правка | `pnpm test` | **69 из 174** |
| Полный — выкладка и CI | `pnpm test:all` | **174** |

Ни одна проверка не потеряна и не ослаблена: полный прогон запускает все 174 теми же
...
путей — `CHANGELOG.md` 2.2.0, складывание папок — 2.3.0). Дерево строится по путям проекта, а не по одним
...
| `src/page/panel.js` | Панель выбора: галочки метрик и файлов, категории, дерево путей проекта (файлы вне отчёта — подписью с причиной, папки — со знаком складывания); перерисовку просит у главы сборки |
...
version` отвечает `2.3.0`, `npm access get status @vernikr/size-report` — `public`,
...
```bash
pnpm add -D github:vernikr/size-report#v2.3.0
```

Без сети (или если тянуть из codeload нечем) — тарболл: `pnpm pack` в клоне
пакета, затем `pnpm add -D ./vernikr-size-report-2.3.0.tgz`.
...
движущаяся цель, а тег постоянен: этот выпуск стоит на теге `v2.3.0`, он же и в
