...
**24,3–25,7 с** при 117 проверках (24,6–24,9 с в среде без настроек git), из них
**+8,5 с** — десять проверок хука (`test/hook.test.js`: сам он идёт 17,9–18,5 с и
становится самым долгим файлом набора, а та же ревизия без него — 15,8–16,6 с при
107 проверках); поэтому цель — `pnpm test`
**≤ 28 с** (запас 2,3 с). Цель поднята один раз и по делу: это интеграционные
прогоны (клон, коммиты, слияние, отказы) — дешевле их не сделать, не ослабив
проверку, — а вклад измерен парным прогоном с файлом и без, а не выведен из
разброса прогонов.
