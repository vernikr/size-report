...
модулей, а последняя — о том, что записанное в манифестах сходится с файлами
эталонов; всего 51.
...
запасом на следующую проверку, а не под сегодняшнее число: набор стоит **6,9 с**
при 51 проверке, поэтому цель — `pnpm test` **≤ 10 с** (запас 3,1 с),
`pnpm run parity:live` **≤ 15 с** (замеры и машина — `REFACTOR.md` §5).
...
| `tools/check-standards.js` | Проверяет, что оба эталона воспроизводятся: пересъём идёт в никуда и сверяется с закоммиченным (`pnpm run check:standards`) |
...
| `src/page/app.css` | Оформление страницы сверх общей части: панель с легендой, состояния пустоты, узкое окно |
...
дают те же файлы, и обе стороны пары закреплены — инструмент это встроенная копия
