...
**Волна 1 начата пунктом R-1.1** (`REFACTOR.md`): вычислительная часть отчёта
одна — страница исполняет те же функции, что считают статическую таблицу
(`DERIVED_SRC`), а не свою копию расчёта; артефакт и разметка страницы при этом
совпали со старым выводом побайтово.
...
```text
dist/app.js        собранная программа отчёта (пока — текст в движке)
...
плана): метрики и оба отчёта — статичный артефакт и страница — считаются на
сборке, а программа страницы вклеивается текстом (`APP_DOM` — разметка,
`DERIVED_SRC` — тот же вычислительный код, что считает артефакт). Остальное — по
шагам 2–6 (`PLAN.md` §5).
