...
**Выпуск 2.0.2 (2026-09-15).** Инструмент живёт отдельным пакетом: имя в
...
version` отвечает `2.0.2`, `npm access get status @vernikr/size-report` — `public`,
...
```bash
pnpm add -D github:vernikr/size-report#v2.0.2
```

Без сети (или если тянуть из codeload нечем) — тарболл: `pnpm pack` в клоне
пакета, затем `pnpm add -D ./vernikr-size-report-2.0.2.tgz`.
...
движущаяся цель, а тег постоянен: этот выпуск стоит на теге `v2.0.2`, он же и в
