...
|---|---|---|
| Быстрый — каждая правка | `pnpm test` | **57 из 134** |
| Полный — выкладка и CI | `pnpm test:all` | **134** |

Ни одна проверка не потеряна и не ослаблена: полный прогон запускает все 134 теми же
...
владельца, репозиторий и файл `release.yml`. Проверить, что вход настроен, можно
черновым прогоном из Actions («Run workflow»: по умолчанию он ничего не публикует,
а только собирает и показывает, что уехало бы) — и это же стережёт
`test/release.test.js` со стороны текста.
...
- **Минификатора нет** (установка без необязательных зависимостей, платформа без
  ...
  готовой починкой. У **проверки** в этом случае свой ответ, и он назван здесь
  потому, что именно её советует CI: если отчёт на диске собран с настоящим
  ...
  (**код 1**), показав разошедшуюся строку подписи. Починка — та же (`pnpm run
  sizes` — и она же вернёт **код 4** на этом окружении); почему первая причина у
  проверки беднее сборки — предмет `REFACTOR.md` R-4.23.
