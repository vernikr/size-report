...
поэтому `pnpm add -D size-report` не сработает. Репозиторий публичный, и
...
пакета, затем `pnpm add -D ./size-report-1.0.0.tgz`.
...
остаётся на потом — публикация в npm (`PLAN.md` §8.4).
