...
состоянии файла нет». Граница, которую нашёл тот же разбор, названа честно: при
`diff.renames=false` и переименовании в псевдоним другой колонки сверка откажет —
там починка меняет числа и идёт отдельно (`BLOCKERS.md` §N8).
...
файлов по папкам, с переключателем у каждой папки на всё поддерево; выбор
читателя переживает перезаход и передаётся ссылкой — адрес страницы и есть ссылка.
...
словарём», две — о сверке с деревом (§«История с удалениями больше не тупик»);
всего 75.
...
**11,0 с** при 75 проверках (три прогона: 10,74 / 10,80 / 11,30, плюс 10,88 в среде
без настроек машины; из них 6,7 с — файл токенов), поэтому цель — `pnpm test`
**≤ 13 с** (запас ~1,7 с), `pnpm run parity:live` **≤ 15 с** (7,4 с в обеих средах).
Почему цель поднята: токены — единственная проверка, которой нужен настоящий
словарь (0,3 с на процесс), а не ослабление сверки; замеры, машина и разброс —
`REFACTOR.md` §5.
...
метрики вместе с теми, которые он берёт, — подпись не может разойтись с тем, что
происходит, и метрика со смешанными форматами объявлена приближённой целиком, а не
выдаёт худшее за точное. Минификатора нет (установка без необязательных
...
причиной: у них число идёт по байтам, и метрика объявлена приближённой целиком —
подпись считается по настройкам, поэтому разойтись с тем, что происходит, не может.
...
| `test/disk.test.js` | Сверка с рабочим деревом: правка только на диске, потерянная правка и потерянное создание (обе — мутацией), файл, удалённый до HEAD, — не потеря (`BLOCKERS.md` §B3) |
| `test/cli.test.js`, `test/cli-paths.test.js` | Отказы командной строки: справка, настройки, коды выхода — и куда инструмент пишет |
| `test/contract.test.js` | Контракт данных и страница: числа против эталона, производные против чисел артефакта, дерево файлов против путей, память выбора и ссылка против перезахода, чужого отчёта и чужого адреса |
...
пометка по файлу   приближение видно по формату в подписи метрики, а не в каждой клетке (шаг 3)
