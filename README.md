...
**10,4 с** при 73 проверках (три прогона: 10,35 / 10,44 / 10,37, плюс 10,48 в среде
без настроек машины; из них 6,7 с — файл токенов), поэтому цель — `pnpm test`
**≤ 13 с** (запас ~2,5 с), `pnpm run parity:live` **≤ 15 с** (7,4 с в обеих средах).
...
цена словаря, а не разбор: таблицы словаря читаются **0,3 с на процесс**, поэтому
набор проверок стал **7,3–7,9 → 10,4 с** при 66 → 73 проверках (запас и новый
бюджет — ниже). Черновик `--init` ведёт новые проекты сразу на токены.
