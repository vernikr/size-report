...
**13,3–13,5 с** при 107 проверках (13,25 с в среде без настроек git), из них
**0,9–1,0 с** — восемь проверок диагностики (`test/doctor.test.js`: та же ревизия
без этого файла идёт 12,4 с при 99 проверках); поэтому цель — `pnpm test`
**≤ 18 с** (запас 4,5 с). Цель не двигалась, хотя числа ниже прошлого прохода:
разница — машина, а не набор (та же ревизия шла и 19,0–24,4 с подряд при загрузке
55…100), и собственный вклад нового файла измерен отдельно, а не выведен из
разброса прогонов.
...
| `src/doctor.js` | Диагностика одним ответом (`size doctor`): окружение, зависимости, настройки, покрытие — сборкой из существующих кусков |
