...
**Выпуск 1.3.1 (2026-09-15).** Инструмент живёт отдельным пакетом: имя в
...
version` отвечает `1.3.1`, `npm access get status @vernikr/size-report` — `public`,
...
```bash
pnpm add -D github:vernikr/size-report#v1.3.1
```

Без сети (или если тянуть из codeload нечем) — тарболл: `pnpm pack` в клоне
пакета, затем `pnpm add -D ./vernikr-size-report-1.3.1.tgz`.
...
движущаяся цель, а тег постоянен: этот выпуск стоит на теге `v1.3.1`, он же и в
