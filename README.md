...
шаблоны называют **путь внутри проекта** (`node node_modules/size-report/bin/size.js`),
а не имя пакета: `npx <имя>` запускает установленный пакет, только пока тот на
месте, а в проекте без него это имя уходит в реестр и запускает чужой пакет
с тем же именем (`R-4.7` — прежнее решение, `R-4.21` — почему оно отменено).
...
|---|---|---|---|
| Быстрый — каждая правка | `pnpm test` | **59 из 133** | **≤ 10 с** |
| Полный — выкладка и CI | `pnpm test:all` | **133** | **≤ 32 с** |
...
публикация — это смена имени, а не только выкладка; какие имена свободны (проверено
запросом к реестру) и что затронет переименование — `PLAN.md` §10.
