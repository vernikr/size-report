...
`pnpm test` сверяет пакет с эталоном побайтово на фикстуре, `pnpm run parity:live` —
на живой истории `safe-resets` (95 строк × 27 колонок, артефакт байт в байт). Ещё не
сделано: интерактивный отчёт, минификация и токены, разбиение движка по файлам
(пункт D1 в плане). Источник инструмента —
