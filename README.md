...
**13,2–14,8 с** при 83 проверках (четыре прогона: 14,76 / 13,34 / 13,19 / 14,27,
плюс 13,21 в среде без настроек машины), поэтому цель — `pnpm test` **≤ 16 с**.
Почему поднята: три новые проверки шаблонов стоят ~1,2 с (одна из них собирает
настоящий отчёт), а сам набор подорожал ещё и от машины — та же ревизия без файла
шаблонов меряется сегодня в **13,4–14,1 с** против 11,1 с прошлого прохода, то есть
цель ≤ 13 с держалась на прежней загрузке, а не только на числе проверок.
