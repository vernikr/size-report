...
названные команды); читатель фактов один — `tools/docs-facts.js`. Заведённый сторож сразу
...
**23,2–24,0 с** при 122 проверках (23,2 с в среде без настроек git). Числа
разных окон несравнимы вовсе: тот же набор при 119 проверках шёл 25,5–25,7 с,
...
| 2 | что-то в вызове или в проекте — **командная строка** (незнакомый ключ, ключ без значения, повтор ключа, два режима сразу, лишнее слово, команда и режим, неизвестная команда, несовместимый ключ, нет ответа в JSON, два ответа сразу, нет коммита); **настройки и проект** (нет файла настроек, настройки не разобраны, настройки неверны, не git-репозиторий, конфиг уже есть); **история** (нет такого коммита, коммит назван неточно); **хук** (чужой хук, чужой core.hooksPath, нечем звать инструмент); **измерение** (файл не JavaScript, минификатор не разобрал) | текст отказа называет причину и готовую команду |
