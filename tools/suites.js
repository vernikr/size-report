...
export const FAST = [
  ...
  { file: 'test/page-view.test.js', why: 'the assembled page in a real DOM: the program pasted in, emptiness, the layout, switching — two assemblies, both on the shared fixture; **deactivated** while the table is rebuilt, each check skipped with its reason at the top of the file' },
  { file: 'test/page-cols.test.js', why: 'the columns of the table in a real DOM: their order, the widths counted for them and the switches reaching them — one page assembly; **deactivated** while the table is rebuilt, the checks skipped with their reason' },
  { file: 'test/page-tree.test.js', why: 'the panel\'s file tree in a real DOM: folders by the project\'s paths, a subtree, scrolling on a rebuild — one page assembly, on the shared fixture; **deactivated** while the table is rebuilt, the checks skipped' },
  { file: 'test/page-choice.test.js', why: 'the memory of a choice and the link in a real DOM: revisits and address changes — it counts in memory, one page assembly; **deactivated** while the table is rebuilt, the checks skipped' },
  { file: 'test/page-grid.test.js', why: 'the grid of the page in a real DOM: the window of rows and columns the reader sees, the geometry of the styling against the script\'s, a switch, a scroll and the numbers under their captions — two page assemblies, both on the shared fixture' },
