...
export const FAST = [
  ...
  { file: 'test/page-view.test.js', why: 'the assembled page in a real DOM: the program pasted in, the block\'s shape, the recount by the choice, the marks and the method, the styling, the empty states and the focus — two assemblies, both on the shared fixture' },
  { file: 'test/page-cols.test.js', why: 'the columns of the grid in a real DOM: their order by the numbers that moved, a caption over its group, and a choice that leaves the others where they are — one page assembly' },
  { file: 'test/page-tree.test.js', why: 'the panel\'s file tree in a real DOM: folders by the project\'s paths, a subtree, files outside the report, the folded tree the memory keeps and a scroll a click does not touch — one page assembly, on the shared fixture' },
  { file: 'test/page-choice.test.js', why: 'the memory of a choice and the link in a real DOM: revisits, refusals, a host without an unpacker and an address that is never rewritten — it counts in memory, one page assembly' },
