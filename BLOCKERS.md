...
- **N10. Обвязка читает git без закреплённых настроек.** В `tools/harness.js`
  `gitIn` зовёт `git -C …` как есть, тогда как движок закрепляет окружение на
  своей границе (`src/git.js`, закрыто в `B1`). Пока это ни на что не влияет: все
  пути самого репозитория ASCII, а числа и вывод инструмента читает не обвязка, а
  сам инструмент. Но проверка, которая читает git напрямую, зависит от машины —
  сторож документации это нашёл первым: с `GIT_CONFIG_GLOBAL=/dev/null`
  `git log --name-only` вернул путь фикстуры закавыченным, и `docs/заметки.md` из
  `BLOCKERS.md` стал «несуществующим» (`quotePath` по умолчанию включён). В самом
  стороже закреплено у его вызова (`-c core.quotePath=false`), потому что иначе
  он падал бы на чужой машине и был бы зелёным на этой. Общее место — тот же
  список закреплений, что у движка; цена — одна строка в `gitIn`.
