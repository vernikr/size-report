...
**Чем держится** (`test/disk.test.js`, две проверки, обе на числах и текстах):

| Проверка | Что утверждает | На старом коде | Мутация, которую она роняет |
|---|---|---|---|
| «файл, удалённый до HEAD, не роняет прогон и числа сходятся с историей» | история с добавлением, правкой, удалением, возвратом и повторным удалением собирается кодом 0, а числа колонки сходятся с размером блоба из git на каждом коммите (возврат даёт то же число, что первое появление) | красная: «состояние «gone.js» на HEAD не совпало с деревом коммита (файла нет вместо файла нет)» | вернуть прежнее условие `p === undefined` — падает ровно она |
| «потерянное создание файла ловится состоянием против дерева» | файл, появившийся **только в слиянии** (проверка сама утверждает, что его нет ни у одного родителя), при потере списка изменённых путей у merge-коммита роняет прогон и называет сторону дерева и сторону состояния | красная только на тексте отказа (ловля была и раньше) | убрать `--diff-merges=first-parent` — падает ровно она |

Ловля потерянной **правки** стережётся там же третьей проверкой (мутированный
движок без первого родителя на фикстуре), она была и остаётся красной на этом
случае.
...
- **N3. Что проверено и оказалось инертным** (чтобы не перепроверять). Каждая
  ...
  | Настройка | Результат |
  |---|---|
  | `diff.renames=true/false` | не влияет на числа — но с `false` включает другой путь отказа, названный в заметке N8 ниже |
...
- **N8. Граница починки B3: псевдоним колонки и `diff.renames`.** Колонка может
  перечислять несколько путей одного файла (`modern.js` — это `src/modern.js` или
  `src/legacy.js`). Путь для состояния берётся из списка псевдонимов, и при
  выключенном распознавании переименований git отдаёт в коммите-переименовании
  **оба** пути: движок выбирает первый по порядку настроек — тот, которого в
  коммите уже нет, — и состояние теряет файл. Воспроизведение (на фикстуре, где у
  колонки `modern.js` два псевдонима):
  ...
  Починка тут — не в сверке, а в выборе пути для состояния (брать псевдоним,
  который в коммите действительно есть), то есть меняет **числа**, — это отдельный
  проход, а не этот. При `diff.renames=true` (умолчание git) переименование даёт
  один путь — новый, — и всё сходится; поэтому дефект узкий: нужны и выключенное
  распознавание, и переименование в псевдоним другой колонки, и это последняя
  правка файла. Проверено прогоном с настройкой и без неё.
