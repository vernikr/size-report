...
## B3. Колонка, чей файл удалён до HEAD, роняет прогон — ОТКРЫТ 2026-09-14
...
**Предлагаемая починка** (одна точка, поведение становится тем, что обещает
комментарий): отказ только когда состояние и дерево **расходятся** — либо у
пустого состояния в дереве есть файл (`aliases.length > 0`), либо у непустого
содержимое другое. Заодно защитить сборку текста: сейчас в той же ветке
`tree.get(p).slice(0, 7)` бросит исключение, если путь из состояния в дереве не
нашёлся (это второй случай того же класса — потерянное **удаление**; проверено
чтением, а не прогоном: такой перенос состояния искусственно не воспроизводится).
Меняется текст отказа, поэтому держать это обязан сценарий в `test/disk.test.js`,
красный до правки.

**Чем подтверждено.** Прогон выше (записан `WORKLOG.md` §27); на живом проекте и
на фикстуре не проявляется — там колонок с удалённым до HEAD файлом нет.
