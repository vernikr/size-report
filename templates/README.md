...
- `fixCommand` — команда, которую цитирует подпись отчёта и отказы. В шаблоне это
  `node node_modules/size-report/bin/size.js --write` — путь к установленному
  пакету внутри проекта. **Имени пакета как команды здесь быть не может:**
  `npx <имя>` в проекте без установленного пакета уходит в реестр и запускает
  **чужой** пакет под тем же именем (в npm `size-report` занято с 2017 года), то
  есть совет, который должен выручать, приводит к чужому коду. Если в проекте есть свой
