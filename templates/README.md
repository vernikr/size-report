...
`size --init` (он подбирает крупнейшие файлы по списку расширений исходников; доки
в этот список не входят) — можно взять его черновик целиком, а из шаблона
перенести ключи, которых `--init` не пишет (`skip`, `rows`, `links.commitUrl`),
или наоборот: скопировать шаблон и вписать колонки руками. Отсюда же растёт
ожидаемое: колонки примерные, поэтому `size check` на свежем проекте скажет, что
